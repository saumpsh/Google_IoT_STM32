...
int GcpIoT_publishTelemetry( gcp_client_t *gcpClient )
{
  ...
  //double _lat = 37.19, _long = -121.52;
  //double _lat = hgps.latitude, _long = hgps.longitude;
  double _lat = 37.3368643, _long = -121.8811451;

  strcat((char *)payload,"},{");
  strcat((char *)payload,"\"dataType\":\"latitude\",\"value\":");
  sprintf(locationbuf,"\"%f\"", _lat);
  strcat ((char *) payload, locationbuf);

  strcat((char *) payload, "},{");
  strcat((char *) payload, "\"dataType\":\"longitude\",\"value\":");
  sprintf(locationbuf, "\"%f\"", _long);
  strcat((char *) payload, locationbuf);
